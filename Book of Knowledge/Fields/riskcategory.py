...
class RiskCategorySearcher:
    def __init__(self):
        self.pattern = re.compile(
            r'\b(?:SEISMIC\s+)?RISK\s+CATEGORY\b\s*[:=]?\s*(I{1,3}|IV|1|2|3|4)\b',
            re.IGNORECASE
        )
    ...
    def search(self, text):
        ...
        filtered = [m.upper() for m in matches if m.upper() in valid_roman or m in valid_numeric]
    ...
    def standardize(self, category):
        ...
        parts = [p.strip().upper() for p in category.split(',')]
